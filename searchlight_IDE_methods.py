...
if __name__ == '__main__':
    ...
    results_outdir = os.path.join(utils.get_scratch_dir(), 'IDE', 'LOSO', 'results')
    ...
    brain_mask = utils.get_intersect_mask(subject_filter=p.subject_filter)
    wb_mask=brain_mask.get_fdata()
    if p.verbose: print(f"Begin Searchlight in rank {rank}")
    all_sl_result = sl.run_searchlight(IDE_kernel, pool_size=pool_size)
    ...
    if rank == 0: 
        ...
        if p.verbose: print(f'results of shape: {np.shape(all_sl_result)}; saved to {output_name}_all_IDE_results_vectorized_rad_{p.sl_rad}.npy')
        N = len(METHODS_OUTPUT_LABELS)
        result_vec = [N*[0] if not n else n for n in result_vec] # replace all None
        
        for i, nm in zip(np.arange(N), METHODS_OUTPUT_LABELS):
            ...
            if p.plot: 
                ...
                title = f'{p.dataset} {p.task} {this_subject} {nm} sl radius={p.sl_rad}'
