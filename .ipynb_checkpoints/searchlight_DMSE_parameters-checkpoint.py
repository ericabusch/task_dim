...
def DMSE_kernel(data, sl_mask, myrad, bcvar):
    data=data[0]
    these_params = bcvar[0]
    ...
    R = []
    for paramset in these_params:
        #print(paramset)
        KNN,T = paramset[1], paramset[2] 
        func = ide.METHODS[paramset[0]]
        R.append(func(data_arr, KNN, T))
...
if __name__ == '__main__':
    ...
    if p.verbose and rank == 0: print(f'loaded {p.dataset}_utils; testing metric={p.metric}')
    ...
    output_name = os.path.join(results_outdir, f'{this_subject}_{p.task}_ide_{p.metric}_') ## THIS will be changed 
    if not p.overwrite:
...
#     PARAMETERS = {'TPHATE_SE':[('TPHATE_SE', 5, 2), ("TPHATE_SE", 10, 2), 
#                   ('TPHATE_SE', 20, 2), ('TPHATE_SE', 20, 5), 
#                   ('TPHATE_SE', 20, 10)], 'DIFF_MAP_SE':[('DIFF_MAP_SE', 5, 2), ("DIFF_MAP_SE", 10, 2), 
#                   ('DIFF_MAP_SE', 20, 2)]}

    PARAMETERS = {'TPHATE_SE':[('TPHATE_SE', 5, 2), ("TPHATE_SE", 50, 2), 
                  ('TPHATE_SE', 50, 10), ('TPHATE_SE', 50, 20), 
                  ('TPHATE_SE', 4, 20)], 'DIFF_MAP_SE':[('DIFF_MAP_SE', 50, 2), ("DIFF_MAP_SE", 4, 2)]}
     
                               
    THIS_PARAMETERS = PARAMETERS[p.metric]
    ...
    if rank == 0:
        ...
        bcvar.append(THIS_PARAMETERS)
        if p.verbose: print(f"Number of WB voxels: {np.sum(wb_mask == 1)}\nRunning subject {this_subject} task {p.task} slrad {p.sl_rad} metric {p.metric}")
    else:
        data.append(None)
    ...
    all_sl_result = sl.run_searchlight(DMSE_kernel, pool_size=pool_size)
    ...
    if rank == 0: 
        ...
        for i, nm in zip(np.arange(N), THIS_PARAMETERS):
        #for i, nm in zip(np.arange(N), ide.METHOD_NAMES):  ## THIS WAS CHANGED
            KNN, T = nm[1], nm[2]
            paramstr = f'knn{KNN}_t{T}'                   
            ...
            out_fn = f'{output_name}_{paramstr}_whole_brain_SL_rad{p.sl_rad}.nii.gz'
            ...
            if p.verbose: print(f"Saved result to {out_fn}")
            if p.plot: 
