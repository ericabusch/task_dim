...
def DSE_kernel(data, sl_mask, myrad, bcvar):
    data=data[0]
    method = bcvar[0]
...
if __name__ == '__main__':
    ...
    max_blk_edge = 5
    pool_size = 2
    if p.verbose: print(f'in rank {rank}, comm={comm}')
    ...
    METHODS_HERE = ['TPHATE_SE_delta', 'PHATE_SE_delta']
    if p.metric not in METHODS_HERE:
        print(f'{p.metric} not in list of {METHODS_HERE}')
        sys.exit(2)
    ...
    output_name = os.path.join(results_outdir, f'{this_subject}_{p.task}_{p.metric}')
    ...
    all_sl_result = sl.run_searchlight(DSE_kernel, pool_size=pool_size)
    ...
    if rank == 0: 
        ...
        # now need to break them down
        result_vec = all_sl_result[coords]
        things_returned = ['optt', 'DSE_t0', 'DSE_optt', 'delta_DSE']
        N = len(things_returned)
        result_vec = [N*[0] if not n else n for n in result_vec] # replace all None
        
        for i, nm in zip(np.arange(N), things_returned):                  
