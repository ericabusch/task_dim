...
def has_repeat_files(subject, task):
    ...
    if task in CNM_MULTIPLE_FILES.keys() and subject in CNM_MULTIPLE_FILES[task].keys():
        return CNM_MULTIPLE_FILES[task][subject]
...
def get_metric_SL_nii_subject(subject, task, metric, file_idx=0, filter_by_age=0, slrad=5):
    ...
    try:
        ...
        print(f'{dirname}/{subject}*{task}*{filestr}*{metric}_whole_brain_SL_rad{slrad}.nii.gz')
        return
    return nib.load(f)
