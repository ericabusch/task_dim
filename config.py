...
RM_INFANT_DATA_DIRS={'sleep':INFANT_SLEEP_DIR, 'aeronaut':INFANT_AERO_DIR, 'mickey':INFANT_MICKEY_DIR}
RM_INFANT_STRING_MATCH={'sleep':'*_fslmotion_thr0.2_Only.nii.gz','aeronaut':'Z','mickey':'Z'}
...
# the ones that were only included in the RM dataset
...
INFANT_SUBJECTS_TASKS = {"sleep": SLEEP_INFANT_SUBJECTS, 
						 "aeronaut":AERO_INFANT_SUBJECTS, 
						 "mickey":MICKEY_INFANT_SUBJECTS} # all mickey infants saw it twice in a row (148 trs) so just include 74
...
CNM_DATA_DIRS = { K:f'{BASE_DIR_CNEUROMOD}/{K}_3mm' for K in CNM_TASKS }
