...
def tphate_kernel(data, sl_mask, myrad, bcvar):
    ...
    op = tphate.TPHATE(verbose=0, n_jobs=-1, smooth_window=2).fit(data_arr)
...
if __name__ == "__main__":
    ...
    test_subject = SUBJECTS[sub_id]
    print(SUBJECTS, test_subject)

    if rank == 0:
        data_i, wb_mask, affine_mat, dimsize = load_data(test_subject, task)
        data.append(data_i)
    ...
    results_path = f'./Narratives/results/dimensionality/sub-{test_subject:02d}_{task}_tphate'
    if rank == 0: 
        result_vec = all_sl_result[coords]
        result_vec = [2*[0] if not n else n for n in result_vec] # replace all None
        np.save(f'{results_path}_temp.npy', result_vec)
        np.save(f'{results_path}_temp1.npy', all_sl_result)
        for i, nm,cmap in zip([0,1], ['optimal_t','autocorrelation'], ['viridis','magma']):
            ...
            title = f'{task} sub {test_subject} {nm} sl radius={sl_rad}'
