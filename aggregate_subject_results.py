...
if __name__ == '__main__':
    ...
    parser.add_argument('-a','--use_atlas',type=int,default=0)
    ...
    if p.dataset.lower() == 'narratives': 
        ...
        import narratives_config as config
    elif p.dataset.lower() == 'adult_restmovie': 
        import adult_restmovie_utils as utils
        import adult_restmovie_config as config
    elif p.dataset.lower() == 'camcan': 
        import camcan_utils as utils
    elif p.dataset.lower() == 'cneuromod': 
        import cneuromod_utils as utils
        import cneuromod_config as config
    elif p.dataset.lower() == 'infant_restmovie': 
        import infant_restmovie_utils as utils
        import infant_restmovie_config as config
    elif p.dataset.lower() == 'partly_cloudy':
        import partlycloudy_utils as utils 
        import partlycloudy_config as config

    else: 
        print(f'{p.dataset} not valid');  sys.exit(1)
    ...
    if p.verbose: print(f'Loading {p.dataset} {p.task} {p.metric}, atlas={p.use_atlas}; n_voxels={np.shape(mask_coords)}')

    if not p.use_atlas:
        vectors = load_SL_data_to_aggregate(all_subjects, mask_coords, p.task, p.metric, filter_by_age=p.subject_filter, with_repeats=False, slrad=p.sl_rad)
    else:
        vectors, maskers = load_parcel_data_to_aggregate(all_subjects, p.task, p.metric, filter_by_age=p.subject_filter, with_repeats=False)
    ...
    os.makedirs(f'{utils.get_results_dir()}/result_volumes', exist_ok=True)
    
    # save results in vector form
    outdir=f'{utils.get_results_dir()}/all_subject_arrays'
    outfn = f'{outdir}/{p.task.lower()}_{p.metric}_atlas{p.use_atlas}_all_subject_results_no_repeats.npy'
    if p.dataset == 'partly_cloudy': outfn=outfn.replace(p.task.lower(), f'subject_filter_{p.subject_filter}')
    np.save(outfn, vectors)

    # save the average
    outfn = f'{outdir}/{p.task.lower()}_{p.metric}_atlas{p.use_atlas}_average_results_no_repeats.npy'
    if p.dataset == 'partly_cloudy': outfn=outfn.replace(p.task.lower(), f'subject_filter_{p.subject_filter}')
    ...
    if not p.use_atlas:
        ...
        outfn = f'{outdir}/{p.task.lower()}_{p.metric}_atlas{p.use_atlas}_all_subjects_no_repeats.nii.gz'
        if p.dataset == 'partly_cloudy': outfn=outfn.replace(p.task.lower(), f'subject_filter_{p.subject_filter}')
        ...
        outfn = f'{outdir}/{p.task.lower()}_{p.metric}_atlas{p.use_atlas}_average_result_no_repeats.nii.gz'
        if p.dataset == 'partly_cloudy': outfn=outfn.replace(p.task.lower(), f'subject_filter_{p.subject_filter}')
    ...
    if p.metric != 'ISC' and p.dataset.lower() in ['cneuromod','infant_rest_movie']:
        if p.verbose: print('rerunning with repeats')
        if not p.use_atlas:
            vectors = load_SL_data_to_aggregate(all_subjects, mask_coords, p.task, p.metric, filter_by_age=p.subject_filter, with_repeats=True, slrad=p.sl_rad)
        else:
            vectors, maskers = load_parcel_data_to_aggregate(all_subjects, p.task, p.metric, filter_by_age=p.subject_filter, with_repeats=True)

        subject_vectors=np.array(vectors)

        if not p.use_atlas:
            cc_img = concat_imgs([vec2vol(subject_vectors[i], brain_mask) for i in range(len(subject_vectors))])
            outfn = f'{outdir}/{p.task.lower()}_{p.metric}_atlas{p.use_atlas}_all_subjects_with_repeats.nii.gz'
            nib.save(cc_img, outfn)
            # average
            avg = np.nanmean(subject_vectors, axis=0)
            avg_nii = vec2vol(avg, brain_mask)
            outfn = f'{outdir}/{p.task.lower()}_{p.metric}_atlas{p.use_atlas}_average_result_with_repeats.nii.gz'
            nib.save(avg_nii, outfn)
        else:
            cc_img = concat_imgs([expand_to_atlas_volume(subject_vectors[i], maskers[i]) for i in range(len(subject_vectors))])
            outfn = f'{outdir}/{p.task.lower()}_{p.metric}_atlas{p.use_atlas}_all_subjects_with_repeats.nii.gz'
